...
Use `credentials` when your API relies on cookie-backed sessions or browser-managed HTTP auth credentials.
...
For cross-origin browser requests, the server must also allow credentialed CORS responses.
