...
Hono framework adapter for the `@livequery/core` ecosystem. Provides middleware,
...
| `src/api-service-linker.ts` | `HonoApiServiceLinker` — wraps core `ApiServiceLinker`, accepts a registry or plain array |
| `src/datasource.ts` | `createDatasourceMapper()` — initialises a typed datasource and returns a `useDatasource()` handler factory |
| `src/index.ts` | Re-exports all of the above plus selected symbols from `@livequery/core` |
...
class HonoApiGateway extends ApiGatewayHandler {
    constructor(options?: { websocketGateway?: WebsocketGateway; discovery?: UdpDiscovery<ServiceApiMetadata> })
}
```

Thin subclass of core `ApiGatewayHandler`. Exposes all core methods including
...
|---|---|---|
| Relationship to core | Subclass | Composition |
| Direct core API access | Yes | Via `.gateway` |
...
Wraps core `ApiServiceLinker`. Accepts a `LivequeryRouteRegistry` (reads `.routes`)
