...
import type { ServiceApiMetadata } from '@livequery/core'
...
describe('HonoApiGateway', () => {
    ...
    test('gateway linker auto-discovers service API through UdpDiscovery', async () => {
        ...
        const serviceDiscovery = new UdpDiscovery<ServiceApiMetadata>({ key, port })
        const gatewayDiscovery = new UdpDiscovery<ServiceApiMetadata>({ key, port })
        const serviceLinker = new HonoApiServiceLinker({
            routes: serviceLivequery.registry,
            discovery: serviceDiscovery,
        })
        const gatewayLinker = new HonoApiGatewayLinker({ discovery: gatewayDiscovery })
