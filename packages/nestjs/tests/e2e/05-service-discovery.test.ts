...
import { API_GATEWAY_NAMESPACE, UdpDiscovery, WebsocketGateway, WEBSOCKET_PATH, type ServiceApiMetadata } from '@livequery/core'
...
function createDiscoveryPair(): [UdpDiscovery<ServiceApiMetadata>, UdpDiscovery<ServiceApiMetadata>] {
    const port = 20_000 + Math.floor(Math.random() * 30_000)
    const options = {
        namespace: API_GATEWAY_NAMESPACE,
        tags: ['livequery'],
        key: 'nestjs-e2e-discovery-key',
        port,
    }
    return [
        new UdpDiscovery<ServiceApiMetadata>(options),
        new UdpDiscovery<ServiceApiMetadata>(options),
    ]
}
...
async function buildServiceApp(gateway: WebsocketGateway, httpServer: http.Server, discovery: UdpDiscovery<ServiceApiMetadata>) {
    @Module({
...
async function buildGatewayApp(gateway: WebsocketGateway, httpServer: http.Server, discovery: UdpDiscovery<ServiceApiMetadata>) {
    @Module({
...
describe('Service discovery — ApiServiceLinker broadcasts, ApiGateway picks up', () => {
    test('gateway proxies request to discovered service', async () => {
        const [svcDiscovery, gwDiscovery] = createDiscoveryPair()
        // --- Service side ---
        ...
        const { app: svcApp } = await buildServiceApp(svcGateway, svcServer, svcDiscovery)
        ...
        const { app: gwApp } = await buildGatewayApp(gwGateway, gwServer, gwDiscovery)
