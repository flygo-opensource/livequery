...
class FakeDiscovery extends ReplaySubject<DiscoveryEvent<ServiceApiMetadata>> {
    readonly broadcasts: DiscoveryMessage<ServiceApiMetadata>[] = []

    constructor() {
        super(20)
    }

    async broadcast(message: DiscoveryMessage<ServiceApiMetadata>): Promise<void> {
        const enriched = { ...message, remote_host: message.remote_host || '127.0.0.1' }
        this.broadcasts.push(enriched)
        this.next(enriched)
    }
...
describe('NestJS core contract', () => {
    ...
    test('ApiServiceLinker broadcasts controller routes in core metadata shape', () => {
        const serviceMessage = discovery.broadcasts.find(message => message.data.role === 'service')
        const serviceMetadata = serviceMessage?.data
        expect(serviceMetadata?.name).toBe('contract-service')
        expect(serviceMessage?.remote_host).toBe('127.0.0.1')
